...
> ⚠️ **문구는 실제 기능과 반드시 일치해야 한다.** 현재 앱에는 커뮤니티 피드가 있고,
> 저널을 공개로 저장하면 다른 독자에게 보인다(공개/비공개 토글). 책 정보 공유 링크도 있다.
> ⚠️ 아래 "나만 보는 기록" 계열 문구는 **커뮤니티가 있는 현재 상태와 맞지 않으므로
> 반드시 고쳐 쓸 것.** 데이터 보안 양식의 "사용자 생성 콘텐츠 공개" 항목도 함께 재검토.
